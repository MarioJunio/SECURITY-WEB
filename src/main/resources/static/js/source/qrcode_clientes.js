$(function() {
	...
	var boxNoRecordsFound = $('.js-no-records-found');
		...
		ajaxSearch = $.ajax({
			...
			data : {
				...
				function(msg) {
					...
					if (!msg.length) {
						$('.js-filter-expression').text(nome.trim());
						tbClientes.fadeOut('fast');
						boxNoRecordsFound.fadeIn('fast');
					} else {
						boxNoRecordsFound.fadeOut('fast');
						tbClientes.fadeIn('fast');
						...
						$.each(msg, function(i, item) {
							...
							var tr = '<tr>';
							tr += '<td style="width: 5%;">' + '<input data-id="' + item.id + '" data="' + item.id + ':'
									+ item.nome + '" id="' + item.id
									+ '_cb" type="checkbox" class="filled-in js-cb-gerar" id="filled-in-box" ' + checkedAttr
									+ '/> <label for="' + item.id + '_cb"></label></td>';
						...
						$('.js-cb-gerar').on('change', function() {
							...
							// marca ou desmarca o checkbox todos, de acordo com
							// o
						...
						// desmarca o checkbox todos selecionados, caso não
						// estejam
						// ao realizar uma nova busca
