...
public interface CheckinRepository extends PagingAndSortingRepository<Checkin, Long> {
	...
	public List<Checkin> findAllOrdered(Pageable pageable);

	@Query("select new br.com.security.reports.VisitaCliente(DATE_FORMAT(a.data, '%d/%m/%Y'), DATE_FORMAT(a.data, '%H:%i:%s'), b.nome, a.status, a.descricao) from Checkin a join a.empregado b join a.cliente c where (c.id = :id) and (a.data between :di and :df) and (a.status in :status) order by a.data asc")
